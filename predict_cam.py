...
# Create output folder for saving frames (optional)
...
while True:
    ...
    results = model.predict(
        ...
        conf=0.7,
        line_width=2,
        classes=[0, 1, 2, 3]  # only relevant objects
    )
    ...
    if key == ord('q'):
        ...
        # Save current frame
        output_path = os.path.join(output_folder, f"camera_frame_{frame_count:04d}.jpg")
        cv2.imwrite(output_path, img)
        print(f"[INFO] Frame saved to {output_path}")
        frame_count += 1
